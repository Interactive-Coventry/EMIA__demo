...
def setup_video_view():
    ...
    with st.container():
        ...
        with col1:
            ...
            df_coord = pd.DataFrame({"camera_id": ["Camera 1 (SG)"], "lng": [103.8198], "lat": [1.300190]})
            ...
            if dashcam_source_btn == "Camera 2 (GR)":  # Dashcam from greece
                ...
                df_coord = pd.DataFrame({"camera_id": ["Camera 2 (GR)"], "lng": [23.405626], "lat": [38.037139]})
            ...
            if dashcam_source_btn == "Camera 3 (UK)":  # Youtube link
                ...
                df_coord = pd.DataFrame({"camera_id": ["Camera  3 (UK)"], "lng": [-1.141297], "lat": [52.601009]})
            ...
            if dashcam_source_btn == "Camera 4 (UK)": # Youtube link
                ...
                st.session_state.stream_url = youtube_url
                st.session_state.update_every_n_frames = 60
                df_coord = pd.DataFrame({"camera_id": ["Camera  4 (UK)"], "lng": [-0.075278], "lat": [51.505554]})

        with col2:
            if st.checkbox("Show camera locations on map", value=False, key="plot_markers_on_map"):
                center_coords = [df_coord.iloc[0]["lat"], df_coord.iloc[0]["lng"]]
                m = plot_markers_on_map(center_coords, df_coord, label_column="camera_id")
