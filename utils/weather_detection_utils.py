...
def predict_weather_class(image_dataset_dir, filename, model=None, labels_map=None, model_name='efficientnet-b7'):
    ...
    img = prepare_image_for_model_input(image_dataset_dir, filename, show_preprocessed=False)
...
def load_weather_detection_model():
    ...
    weather_detection_folder = core_utils.settings['MODELS']['weather_detection_folder']
    weather_detection_checkpoint_file = core_utils.settings['MODELS']['weather_detection_checkpoint_file']
    weather_detection_model = core_utils.settings['MODELS']['weather_detection_model']
    ...
    print(f'New weather classification model loaded from {weather_detection_checkpoint_file}.\n')
...
def make(base_model_name, weight_path, class_mapping, version, has_augmentation=False, has_transfer_learning=True):
    ...
    target_model_class = train_with_lightning.PredictionModel
